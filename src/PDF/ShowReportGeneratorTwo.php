...
class ShowReportGeneratorTwo
{
    ...
    public function generate($transactions, $start_date, $end_date): string
    {
        ...
        foreach ($transactions as $t) {
            ...
            $pdf->Cell(6, 6, $t->id, 1, 0, 'C');
            $pdf->Cell(20, 6, substr($t->shipper->name, 0, 12), 1, 0, 'C');
            $pdf->Cell(20, 6, substr($t->exhibitor->name, 0, 12), 1, 0, 'C');
            $pdf->Cell(10, 6, substr($t->carrier->name, 0, 8), 1, 0, 'C');
            $pdf->Cell(20, 6, substr($t->tracking, 0, 12), 1, 0, 'C');
            $pdf->Cell(20, 6, substr($t->client->name, 0, 12), 1, 0, 'C');
            $pdf->Cell(15, 6, substr($t->showPlace->name, 0, 12), 1, 0, 'C');
            $pdf->Cell(9, 6, substr($t->booth, 0, 8), 1, 0, 'C');
            $pdf->Cell(9, 6, $items['crate']->pcs ?? 0, 1, 0, 'C');
            $pdf->Cell(9, 6, $items['carton']->pcs ?? 0, 1, 0, 'C');
            $pdf->Cell(9, 6, $items['skid']->pcs ?? 0, 1, 0, 'C');
            $pdf->Cell(9, 6, $items['fiber_case']->pcs ?? 0, 1, 0, 'C');
            $pdf->Cell(9, 6, $items['carpet']->pcs ?? 0, 1, 0, 'C');
            $pdf->Cell(9, 6, $items['misc']->pcs ?? 0, 1, 0, 'C');
            $pdf->Cell(10, 6, $total_pcs, 1, 0, 'C');
            $pdf->Cell(10, 6, $total_weight, 1, 0, 'C');
            $pdf->Cell(10, 6, $billable_weight, 1, 0, 'C');
            $pdf->Cell(13, 6, substr($t->shipment, 0, 12), 1, 0, 'C');
            $pdf->Cell(10, 6, substr($t->receiver, 0, 12), 1, 0, 'C');
            $pdf->Cell(10, 6, substr($t->trailer, 0, 12), 1, 0, 'C');
            $pdf->Cell(22, 6, $t->created_at, 1, 0, 'C');
            $pdf->Ln();
            $pdf->Cell(100, 6, ' ' . $t->shipper->city . ',  ' . $t->shipper->state . ',  ' . $t->shipper->zip, 1, 0, 'L');

            $pdf->Cell(117, 6, substr($notes, 0, 100), 1, 0, 'C');
    ...
    private function placeTableHeader($pdf) {
        ...
        $pdf->Cell(20, 6, 'Shipper', 1, 0, 'C');
        $pdf->Cell(20, 6, 'Exhibitor', 1, 0, 'C');
        $pdf->Cell(10, 6, 'Carrier', 1, 0, 'C');
        $pdf->Cell(20, 6, 'Tracking', 1, 0, 'C');
        $pdf->Cell(20, 6, 'Client', 1, 0, 'C');
        $pdf->Cell(15, 6, 'Zone', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Booth', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Crate', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Carton', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Skid', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Fbr Cse', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Carpet', 1, 0, 'C');
        $pdf->Cell(9, 6, 'Misc', 1, 0, 'C');
        $pdf->Cell(10, 6, 'Total Pcs', 1, 0, 'C');
        $pdf->Cell(10, 6, 'Weight', 1, 0, 'C');
        $pdf->Cell(10, 6, 'Billable', 1, 0, 'C');
        $pdf->Cell(13, 6, 'Shipment', 1, 0, 'C');
