...
class DocGenerator
{
	 ...
	 */
	public function __construct(string $orientation = 'P', string $unit = 'mm', array|string $format = [210, 297])
	{
		// orientation, unit, format
		$this->pdf = new DocPDF($orientation, $unit, $format);
		$this->setMetaData();
		$this->configPDF();
	}
	...
	public function configPDF(
		float $leftMargin = PDF_MARGIN_LEFT,
		float $topMargin = 10,
		float $rightMargin = PDF_MARGIN_RIGHT,
		float $headerMargin = PDF_MARGIN_HEADER,
		float $footerMargin = PDF_MARGIN_FOOTER
	): void {
		// Set margins
