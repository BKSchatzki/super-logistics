...
const startingPage = computed(() =>
  Object.keys(props.labelData).length === 0 ? "1" : "4",
);
...
const validationSchema = yup.object().shape({
  ...
  remarks: yup.string().when("special_handling", {
    is: true,
    then: (schema) =>
      schema.required("Remarks are required when Special Handling is checked"),
  ...
  image: yup.mixed().when("image_path", {
    is: (value) => !value,
    then: (schema) =>
      schema.required("Please take a picture of the shipment").label("Image"),
        ...
        <StepPanel v-slot="{ activateCallback }">
          <Col>
            ...
            <Row>
              <FormTextarea
                name="tracking"
                label="Tracking Number or Pro Number"
              ...
              <Row v-else>
                <Col>
                  ...
                  >
                    <img
                      :src="preview"
                      alt="Preview of the Transaction"
  ...
  <Col v-else>
    <div class="flex flex-col items-center gap-4">
      ...
      <span class="text-3xl font-light"
        >Receiver {{ values["id"] }} Submitted</span
      >
      ...
      <Button
        @click="close"
        severity="secondary"
        :label="onReceiversPage ? 'Close' : 'Scan Again'"
