...
// Mock store
const createVuexStore = (isInternal = true, isInternalAdmin = true) =>
  createStore({
    state: {
      user: ref({
        isInternal,
...
vi.mock("@utils/composables/useFormAssist", () => ({
  useFormAssist: () => ({
    ...
    getDroptions: () => [{ label: "Client 1", value: 1 }],
  }),
}));
    ...
    return mount(ShowForm, {
      ...
      global: {
        ...
        stubs: {
          ...
          Button: {
            template:
              '<button :type="type" :data-test="$attrs[\'data-test\']" @click="$emit(\'click\')">{{label}}</button>',
            props: ["label", "severity", "type"],
            emits: ["click"],
          },
          FormTextInput: {
            template:
              '<div><input :name="name" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /></div>',
            props: ["modelValue", "name", "label"],
            emits: ["update:modelValue"],
          },
          FormSelectInput: {
            template:
              '<div><select :name="name" :value="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)"><option v-for="opt in options" :key="opt.value" :value="opt.value">{{opt.label}}</option></select></div>',
            props: ["modelValue", "options", "name", "label"],
            emits: ["update:modelValue"],
          },
          FormNumberInput: {
            template:
              '<div><input type="number" :name="name" :value="modelValue" @input="$emit(\'update:modelValue\', Number($event.target.value))" /></div>',
            props: ["modelValue", "name", "label"],
            emits: ["update:modelValue"],
          },
          DateInput: {
            template:
              '<div><input type="date" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /></div>',
            props: ["modelValue", "label", "mode", "inline"],
            emits: ["update:modelValue"],
          },
          ShowTextarea: {
            template:
              '<div><textarea :name="name" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)"></textarea></div>',
            props: ["modelValue", "name", "label"],
            emits: ["update:modelValue"],
          },
          Row: {
  ...
  describe("Conditional Rendering", () => {
    it("should show client field when user is internal", async () => {
      ...
      const clientRow = rows.find((row) =>
        row.find('select[name="client_id"]').exists(),
      );
    ...
    it("should show carat weight fields when user is internal admin", async () => {
      ...
      const caratRow = rows.find((row) => {
        ...
        return (
          row_el.querySelector('input[name="min_carat_weight"]') &&
