...
// Deletion
const deleteSubject = () => {
  trash({ id: props.subject.id }).then(triggerUnselect);
};
const restoreSubject = () => {
  restore({ id: props.subject.id }).then(triggerUnselect);
};

// Marking as Inactive
const markSubjectInactive = () => {
  markInactive({ id: props.subject.id }).then(triggerUnselect);
};
const markSubjectActive = () => {
  markActive({ id: props.subject.id }).then(triggerUnselect);
};
...
<template>
  ...
  <Dialog
    ...
    @update:visible="triggerUnselect"
    modal
    contentClass="w-[400]px min-h-60"
    :draggable="false"
  >
    <template #header>
      <template v-if="status === 'viewing'">
        ...
        <Button
          v-if="allowArchive && !!subject['active'] && !subject['trashed']"
          variant="text"
          severity="secondary"
          icon="pi pi-folder"
          rounded
          @click="() => (status = 'archiving')"
        />
        <Button
          v-if="allowEdit && !!subject['active'] && !subject['trashed']"
          variant="text"
          severity="secondary"
          icon="pi pi-pencil"
          rounded
        ...
        />
        <Button
          v-if="allowArchive && !subject['active']"
          variant="text"
          severity="secondary"
          icon="pi pi-folder-open"
          rounded
          @click="() => (status = 'activating')"
        />
        <Button
          v-if="allowTrash && !subject['trashed']"
          variant="text"
          severity="danger"
          icon="pi pi-trash"
          rounded
        ...
        <Button
          ...
          icon="pi pi-undo"
          rounded
          @click="() => (status = 'restoring')"
        />
      </template>
      <template v-if="status === 'deleting'">
        <span class="p-dialog-title text-red-400"
          >Delete {{ niceSingular }}</span
        >
      </template>
      <template v-else-if="status !== 'viewing'">
        <div class="flex flex-row gap-2">
          <span class="p-dialog-title">{{ verb }} {{ niceSingular }} </span>
          <span class="p-dialog-title font-extralight">{{
            props.subject.user_login
          }}</span>
        </div>
      </template>
    </template>
    ...
    <div v-if="status === 'deleting'" class="flex flex-col gap-2">
      <Col>
        <p class="text-red-400">
          Are you sure you want to delete {{ props.subject[nameIDKey] }}?
        </p>
        <div class="flex justify-end flex-row gap-2">
          <Button
            label="Cancel"
            severity="secondary"
            @click="() => (status = 'viewing')"
          />
    ...
    <div v-if="status === 'editing'" class="flex flex-col gap-2">
      <slot
        name="edit-form"
        :formData="props.subject"
        :close="() => (status = 'viewing')"
      />
    </div>
  </Dialog>
</template>
