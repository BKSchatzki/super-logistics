...
export function useFormAssist(staticFormData = {}) {
  ...
  const getDroptions = (topic, params) => {
    ...
    return computed(() =>
      data.value.map((d) => ({ label: d.name, value: parseInt(d.id) })),
    );
  };
  ...
  const getRoleDroptions = () => {
    ...
    return computed(() =>
      user.value["isInternal"] ? internalRoleOptions : clientRoleOptions,
    );
  };
  ...
  const submitToAPI = async (topic, data, method = "post", refresh = true) => {
    ...
    if (!request) {
      console.error(
        "Improper method provided while making submit request. Method provided: ",
        method,
      );
      return;
    }
