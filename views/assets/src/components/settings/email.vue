        ...
        <div class="metabox-holder">
            <div id="pm_mails" class="group" style="">
                <form @submit.prevent="saveEmailSettings()" method="post" action="options.php">
                    ...
                    <h2>{{ __( 'E-Mail Settings', 'super-logistics') }}</h2>
                    <table class="form-table">
                        <tbody>
                            <tr>
                                <th scope="row">
                                    <label for="pm_mails[email_from]">{{ __( 'From Email', 'super-logistics') }}</label>
                                </th>
                                <td>
                            ...
                            <tr>
                                <th scope="row">
                                    <label for="pm_mails[email_url_link]">{{ __( 'Links in the Email', 'super-logistics') }}</label>
                                </th>
                                <td>
                                    <fieldset>
                                        <label for="wpuf-pm_mails[email_url_link][backend]">
                                            <input v-model="link_to_backend" type="checkbox" class="radio">&nbsp;{{ __( 'Link to Backend', 'super-logistics') }}
                                        </label>
                                        <br>
                                        <p class="description">{{ __( 'Select where do you want to take the user. Notification emails contain links.', 'super-logistics') }}
                                        </p>
                                    </fieldset>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <label for="pm_mails[email_type]">{{ __( 'E-Mail Type', 'super-logistics') }}</label>
                                </th>
                                <td>
                                    <select v-model="email_type" class="regular" name="pm_mails[email_type]" id="pm_mails[email_type]" style="width: 120px;">
                                        <option value="text/html">{{ __( 'HTML Mail', 'super-logistics') }}</option>
                                        <option value="text/plain" selected="selected">{{ __( 'Plain Text', 'super-logistics') }}</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <label for="pm_mails[email_bcc_enable]">{{ __( 'Send email via Bcc', 'super-logistics') }}</label>
                                </th>
                                <td>
                                    <fieldset>
...
<script>
    ...
    export default {
        data () {
            return {
                from_email: this.getSettings('from_email', SL_Vars.current_user.data.user_email),
